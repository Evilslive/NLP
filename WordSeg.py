'''
Base On
A.試著不用Jieba，做出更好的斷詞等中文語料處理分析系統。
B.僅切分出1-4字詞(單字詞、雙字詞、三字詞、四字詞)，五、六字詞以上多數可以再切短一點。
C.以文章的用字遣詞計算共現性，使得斷詞不受世代效果的影響，但切出的字詞受寫作習慣影響(作者風格?)。

Issues
a.數量詞: 如「七輛囚車、三輛囚車」，都切為數字+輛囚車。或許中文的數字「一、二、三...」，要獨立拉出來。
b.介詞:的、是、把，透過探討可能的組合詞，先行切出。
c.為了解決a&b的問題，日後增建"rf"版本，新增自定義字典，將預切的字詞放入(那就又回到Jieba的模式了)。
d.處理速度的問題，目前的處理方式會形成檔案四倍大的矩陣，再計次、整合上會較慢
'''
